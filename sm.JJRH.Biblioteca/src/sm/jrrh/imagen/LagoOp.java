 ...
 * en la mitad inferior de la imagen pero de manera inversa consiguiendo el efecto de que estuviera
 * reflejada en agua. Esta clase hereda de BufferedImageOpAdapter sobre la cual se sobrecarga el método filter. 
 * @author julius
 */
public class LagoOp extends BufferedImageOpAdapter{ 
     ...
     * y además en la imagen de salida se copiarán desde el final hacia arriba creando un efecto lago.
    ...
    public BufferedImage filter(BufferedImage in, BufferedImage out) {
        ...
        for (int x = 0; x < in.getWidth(); x++) {
            for (int y = 0; y < in.getHeight(); y++) {
                ...
                if(y<=(in.getHeight()/2)){
                    out.setRGB(x, (in.getHeight()-1) -y, rgb.getRGB());
